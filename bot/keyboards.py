...
def main_menu_keyboard(lang: str) -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(text=t("menu_products", lang)),
                KeyboardButton(text=t("menu_faq", lang)),
            ],
            [
                KeyboardButton(text=t("menu_orders", lang)),
                KeyboardButton(text=t("menu_contact", lang)),
            ],
        ],
...
def back_to_menu_keyboard(lang: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=t("back_to_menu", lang), callback_data="back_to_menu")]
        ]
    )


def product_card_keyboard(
    lang: str, index: int, total: int, product_id: int
) -> InlineKeyboardMarkup:
    nav_row = []
    if index > 0:
        nav_row.append(
            InlineKeyboardButton(text=t("prev_button", lang), callback_data=f"product:{index - 1}")
        )
    if index < total - 1:
        nav_row.append(
            InlineKeyboardButton(text=t("next_button", lang), callback_data=f"product:{index + 1}")
        )
    ...
    rows.append(
        [
            InlineKeyboardButton(
                text=t("order_button", lang), callback_data=f"order_start:{product_id}"
            )
        ]
    )
...
def quantity_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=str(n), callback_data=f"qty:{n}") for n in range(1, 6)]
        ]
    )
...
def payment_options_keyboard(lang: str, order: Order) -> InlineKeyboardMarkup | None:
    ...
    if stripe_provider.enabled:
        rows.append(
            [
                InlineKeyboardButton(
                    text=t("pay_with_card", lang), callback_data=f"pay_stripe:{order.id}"
                )
            ]
        )
    rows.append(
        [
            InlineKeyboardButton(
                text=t("pay_with_stars", lang), callback_data=f"pay_stars:{order.id}"
            )
        ]
    )
    return InlineKeyboardMarkup(inline_keyboard=rows)
